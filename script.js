...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const themeStorageKey = 'academic-homepage-theme';
  const backgroundStorageKey = 'academic-homepage-custom-bg';
  ...
  function syncThemeChipState(activeTheme) {
  ...
  function updateThemeStatus() {
    ...
    const currentTheme = visualThemeLabels[body.dataset.visualTheme] || visualThemeLabels.cyber;
    const backgroundLabel = body.classList.contains('has-custom-background')
      ? '已启用自定义背景'
      : '使用内置场景';
  ...
  function applyTheme(theme, persist = true) {
    const nextTheme = visualThemeLabels[theme] ? theme : 'cyber';
    body.dataset.visualTheme = nextTheme;
    syncThemeChipState(nextTheme);
    updateThemeStatus();
  ...
  function applyCustomBackground(imageUrl, persist = true) {
    if (!imageUrl) return;
    rootStyle.setProperty('--hero-custom-image', `url("${imageUrl}")`);
    body.classList.add('has-custom-background');
    updateThemeStatus();

    if (persist) {
      storeValue(backgroundStorageKey, imageUrl);
    }
  }

  function clearCustomBackground(persist = true) {
    rootStyle.removeProperty('--hero-custom-image');
    body.classList.remove('has-custom-background');
    updateThemeStatus();

    if (persist) {
      removeStoredValue(backgroundStorageKey);
    }
  }
  ...
  function updateHeroClock() {
    ...
    if (heroDate) {
      ...
      heroDate.textContent = `${dateParts.year} 年 ${dateParts.month} 月 ${dateParts.day} 日 ${dateParts.weekday}`;
    }

    if (heroTime) {
      const timeParts = toPartMap(
        new Intl.DateTimeFormat('zh-CN', {
          timeZone: 'Asia/Shanghai',
          hour: '2-digit',
          minute: '2-digit',
      ...
      heroTime.textContent = `${timeParts.hour}:${timeParts.minute}:${timeParts.second}`;
    }
  }

  const savedTheme = readStoredValue(themeStorageKey) || body.dataset.visualTheme || 'cyber';
  const savedBackground = readStoredValue(backgroundStorageKey);

  applyTheme(savedTheme, false);
  if (savedBackground) {
    applyCustomBackground(savedBackground, false);
  } else {
    updateThemeStatus();
  }
  updateHeroClock();
